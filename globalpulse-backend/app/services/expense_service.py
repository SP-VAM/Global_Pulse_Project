...
from app.db.models.expense_model import (
    BudgetModel,
...
class ExpenseService:
    ...
    async def get_or_create_category_by_name(
        ...
        stmt = select(ExpenseCategoryModel).where(
            func.lower(ExpenseCategoryModel.category_name)
            == category_name.lower()
        )
    ...
    async def get_monthly_summary(
        ...
        expenses = await self.expense_repo.get_user_expenses_by_month(
            user_id,
            year,
            month,
        )

        incomes = await self.income_repo.get_user_incomes_by_month(
            user_id,
            year,
            month,
        )

        budgets = await self.budget_repo.get_user_budgets_by_month(
            user_id,
            year,
            month,
        )
        ...
        for exp in expenses:
            ...
            spending_by_category[cat_id] = (
                spending_by_category.get(cat_id, 0.0)
                + float(exp.amount)
            )
        ...
        for budget in budgets:
            cat_spent = spending_by_category.get(
                budget.category_id,
                0.0,
            )
        ...
        return ExpenseSummaryResponse(
            ...
            incomes=[
                IncomeResponse.model_validate(i)
                for i in incomes
            ],
            budgets=[
                BudgetResponse.model_validate(b)
                for b in budgets
            ],
            categories=[
    ...
    async def create_expense(
        ...
        if not cat_id and req.category_name:
            cat = await self.get_or_create_category_by_name(
                req.category_name
            )
        ...
        # Trigger successful expense transaction notification
        cat_name = req.category_name or "Expense"

        try:
            from app.services.notification_service import NotificationService

            cat_stmt = select(ExpenseCategoryModel).where(
                ExpenseCategoryModel.category_id == cat_id
            )
            ...
            await notif_svc.create_and_send_notification(
                ...
                message=(
                    f"Your expense of ₹{float(req.amount):,.2f} "
            ...
            logger.debug(
                "Expense transaction notification skipped: %s",
                exp_notif_err,
            )

        # Trigger unusual account activity notification
        try:
            exp_date = req.expense_date or date.today()

            month_exps = (
                await self.expense_repo.get_user_expenses_by_month(
                    user_id,
                    exp_date.year,
                    exp_date.month,
                )
            ...
            if len(month_exps) > 1:
                ...
                is_unusual = (
                    (
                        avg_spent > 0
                        and float(req.amount) >= 3 * avg_spent
                        and float(req.amount) >= 5000.0
                    )
                    or float(req.amount) >= 100000.0
                )
            ...
            if is_unusual:
                ...
                await notif_svc.create_and_send_notification(
                    ...
                    message=(
                        f"An unusually large expense of "
                        f"₹{float(req.amount):,.2f} was recorded "
            ...
            logger.debug(
                "Unusual activity notification skipped: %s",
                unusual_notif_err,
            )
        ...
        try:
            ...
            if (
                matching_budget
                and float(matching_budget.budget_amount) > 0
            ):
                month_exps = (
                    await self.expense_repo.get_user_expenses_by_month(
                        user_id,
                ...
                if total_spent >= budget_limit:
                    ...
                    await notif_svc.create_and_send_notification(
                        ...
                        message=(
                            f"You have spent ₹{total_spent:,.2f} "
                            f"exceeding your monthly budget limit "
            ...
            logger.debug(
                "Budget notification trigger skipped: %s",
                budget_notif_err,
            )
    ...
    async def create_income(
        ...
        try:
            ...
            await notif_svc.create_and_send_notification(
                ...
                message=(
                    f"Your income of ₹{float(req.amount):,.2f} "
            ...
            logger.debug(
                "Income transaction notification skipped: %s",
                inc_notif_err,
            )
    ...
    async def update_expense(
        ...
        updated = await self.expense_repo.update(
            expense_id,
            updates,
        )
    ...
    async def update_income(
        ...
        updated = await self.income_repo.update(
            income_id,
            updates,
        )
    ...
    async def delete_budget(
        ...
        """Delete a budget record belonging to the user."""
    ...
    async def get_filtered_transactions(
        ...
        include_expenses = params.transaction_type in (
            None,
            "expense",
        )
