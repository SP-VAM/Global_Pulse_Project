"""
Helper: writes all Category hardening test files.
Run from the backend root: python scripts/write_category_tests.py
"""
import os

BASE = os.path.join("tests", "unit")

# ============================================================
# CATEGORY 1 — Authorization & User Isolation
# ============================================================
CAT1 = r'''"""
...
def make_mock_expense(expense_id, user_id):
    m = MagicMock()
    m.expense_id = expense_id; m.user_id = user_id; m.amount = 150.0
    m.expense_date = date(2026, 8, 6); m.payment_method = "UPI"
    m.notes = "Private"; m.created_at = dt_now; m.category_id = 1
    return m


def make_mock_income(income_id, user_id):
    m = MagicMock()
    m.income_id = income_id; m.user_id = user_id; m.amount = 5000.0
    m.income_date = date(2026, 8, 1); m.payment_method = "Salary"
    m.notes = "Monthly"; m.created_at = dt_now
    return m


def make_mock_budget(budget_id, user_id):
    m = MagicMock()
    m.budget_id = budget_id; m.user_id = user_id; m.category_id = 1
    m.budget_amount = 1000.0; m.budget_month = 8; m.budget_year = 2026
    m.created_at = dt_now
    return m
...
def make_mock_investment(investment_id, user_id):
    m = MagicMock()
    m.investment_id = investment_id; m.user_id = user_id
    m.asset_type = "EQUITY"; m.ticker = "RELIANCE.NS"
    m.company_name = "Reliance"; m.quantity = 10; m.purchase_price = 2500.0
    m.purchase_date = date(2026, 1, 1); m.exchange = "NSE"
    m.broker_name = None; m.investment_source = None
    m.notes = None; m.created_at = dt_now
    return m
...
class TestPortfolioServiceUserIsolation:
    def _svc(self):
        ...
        s.session = AsyncMock(); s.portfolio_repo = AsyncMock(); s.stock_provider = AsyncMock()
        return s
...
class TestAuthIdentityIsolation:
    def test_token_encodes_correct_user_id(self):
        ...
        pa = decode_token(ta); pb = decode_token(tb)
...
# ============================================================
files = {
    "test_category1_authorization.py": CAT1,
}
